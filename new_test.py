...
#   CSV_URL = (f'https://api.thingspeak.com/channels/{id_brizo}/feeds.csv?start={start_date}&end={end_date}&api_key={api_key}')
    try:
      with requests.Session() as s:
        ...
        #   )
            df = pd.read_csv('temp/feeds5.csv')
            print(f'Sto processando Brizio_2')
            ...
            df.to_csv('./temp/1.csv', index=False)
            ...
            mission_value = lambda row: f"mission_Brizo_2_{row['time1'].date()}"
            ...
            df = df.assign(platformcode='brizio2')

            today = date.today()
            nome_file_csv=f'estrazione_brizio1_{today}'
            print('before', df)
            # conversione coordinate
            for index, row in df.iterrows():
                df.at[index, 'latitude'] = convert_coordinate(row['latitude'])
                df.at[index, 'longitude'] = convert_coordinate(row['longitude'])
            # d=df['latitude'].div(100)%1
            # df['latitude'] = (df['latitude'].div(100)-(df['latitude'].div(100)%1)) + ((df['latitude'].div(100)%1)*100)/60
            
            # f=df['longitude'].div(100)%1
            # df['longitude'] = (df['longitude'].div(100)-(df['longitude'].div(100)%1)) + ((df['longitude'].div(100)%1)*100)/60
